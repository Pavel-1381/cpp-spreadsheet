...
public:
    ...
    virtual double Evaluate(const ReceiveCell& cell_lookup) const = 0;
...
public:
    ...
    double Evaluate(const ReceiveCell& cell_by_position) const override {
        using namespace std::literals;
        double res = 0.;
        switch (type_) {
        case Add: {
            res = lhs_->Evaluate(cell_by_position) + rhs_->Evaluate(cell_by_position);
            break;
        }
        case Subtract: {
            res = lhs_->Evaluate(cell_by_position) - rhs_->Evaluate(cell_by_position);
...
double FormulaAST::Execute(const ReceiveCell& cell_by_position) const {
    return root_expr_->Evaluate(cell_by_position);
}
